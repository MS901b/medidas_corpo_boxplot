	...
	*/
	...
	if (valores.length>1) {
		...
		for (var i=1;i<=valores.length;i++) {
			...
			applet.setColor('C'+i,cores[i-1][0],cores[i-1][1],cores[i-1][2]);
			...
			applet.setColor('dado'+i+'Label',cores[i-1][0],cores[i-1][1],cores[i-1][2]);
...
function histograma(labels,valores){
	...
	applet.setAxesVisible(false,true)
	...
	*/
	...
	for (var i=0;i<valores.length;i++)
	{
		...
		applet.setColor('dado'+i+'Poly', 255, 0,0)
		applet.setFilling('dado'+i+'Poly',0.3);	
...
function updateListenerPontoControle(objName) {
		...
		if (exibeValorDadosHistograma == true) {	
			...
			var resultado = dadoDoQuantil(minhaTemporaria);

			applet.evalCommand('dadosLabel=Text[('+resultado+'),(x(PontoControle)+0.2,-0.05)]');
...
function barras(labels,valores){
	...
	*/
	...
	for (var i=0;i<valores.length;i++)
	{
		...
		applet.evalCommand('dado'+i+'Poly=Polygon['+nomesPontos[0]+','+nomesPontos[1]+','+nomesPontos[2]+','+nomesPontos[3]+']');
		applet.setFixed('dado'+i+'Poly',true);
		applet.setColor('dado'+i+'Poly',cores[i][0],cores[i][1],cores[i][2]);
...
function larguraChar(char) {
	...
	console.log('Caracter não encontrando, assumindo largura 7');
	return 7;
}
*/
...
function larguraLabel(label) {
	...
	var relacaoLargura = larguraGgb/larguraApplet;
		...
		{
			tamanhoLabel += larguraChar(label[iterLabel])+1;
		}
